...
import { UserModel, BookModel, LoanModel, ReservationModel } from '../models/index.js';
...
async function returnBook(userId, bookId) {
  ...
  try {
    const user = await UserModel.findById(userId).session(session);
    const book = await BookModel.findById(bookId).session(session);
    const loan = await LoanModel.findOne({ user: userId, book: bookId, status: 'borrowed' }).session(session);
    ...
    user.borrowedBooks = user.borrowedBooks.filter(borrowed => borrowed.book.toString() !== bookId);
    await user.save({ session });
    ...
    if (book.reservedBy.length > 0) {
      ...
      const reservation = await ReservationModel.findOne({ user: nextUserId, book: bookId }).session(session);
      
      if (nextUser && reservation) {
        reservation.status = 'fulfilled';
        await reservation.save({ session });

        nextUser.reservations = nextUser.reservations.filter(res => res.book.toString() !== bookId);
...
async function reserveBook(userId, bookId) {
  ...
  const hasBorrowed = user.borrowedBooks.some(loan => loan.book.toString() === bookId);
  ...
  try {
    ...
    if (book.availability) {
      throw new Error("Book is currently available and cannot be reserved.");
    }
    if (hasBorrowed) {
...
async function renewBook(userId, bookId) {
  ...
  try {
    ...
    return { message: "Book renewed successfully.", newDueDate: newDueDate };
  } catch (error) {
    await session.abortTransaction();
    session.endSession();
